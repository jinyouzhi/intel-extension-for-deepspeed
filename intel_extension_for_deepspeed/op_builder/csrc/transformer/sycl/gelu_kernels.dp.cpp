...
void gelu_kernel(const float* input, float* vals, int row_stride, int iterations,
                 nd_item<3> item_ct1)
{
                 ...
                 nd_item<3> item_ct1)
{
    ...
    for (int i = 0; i < iterations; i++) {
        if (i * loop_stride + id < row_stride) {
            ...
            float4 gelu_input_data = {
                bf16::to_float(gelu_input_vec.x()),
...
#pragma unroll
    for (int i = 0; i < iterations; i++) {
        if (i * loop_stride + id < row_stride) {
            ...
            float2 output_half_0 = output_data_half[0].convert<float>(); // __half22float2(output_data_half[0]);
            float2 output_half_1 = output_data_half[1].convert<float>(); // __half22float2(output_data_half[1]);

            float2 gelu_input_half_0 = gelu_input_data_half[0].convert<float>(); // __half22float2(gelu_input_data_half[0]);
            float2 gelu_input_half_1 = gelu_input_data_half[1].convert<float>(); // __half22float2(gelu_input_data_half[1]);
                      ...
                      queue* stream)
{
    ...
    stream->submit([&](handler& cgh) {
        cgh.parallel_for(
            nd_range<3>(grid_dims * block_dims, block_dims), [=](nd_item<3> item_ct1) {
                 ...
                 queue* stream)
{
    ...
    /*
      DPCT1049:1: The workgroup size passed to the SYCL kernel may exceed the limit. To get the
      * device limit, query info::device::max_work_group_size. Adjust the workgroup size if needed.
