...
#pragma unroll
    ...
    if (sg.get_local_id() < (iteration_stride >> 5))
        sum = shr[sg.get_local_id()];
...
#pragma unroll
    ...
    if constexpr(is_mean) {
        if (training)
            if (sg.get_local_id() == 0)
                means_cast[row] = bf16::from_float(mean);
    }
    float variance = 0.f;
    for (int i = 0; i < iterations; i++) {
        vals_arr[i] -= mean;
    ...
    if (training)
        if (sg.get_local_id() == 0)
            vars_cast[row] = bf16::from_float(variance);

    iterations = row_stride / iteration_stride;
    for (int i = 0; i < iterations; i++) {
        vals_arr[i] = vals_arr[i] * rsqrt(variance);
        vals_arr[i] =
            vals_arr[i] * bf16::to_float(gamma_cast[i * iteration_stride + id]) +
            bf16::to_float(beta_cast[i * iteration_stride + id]);
        vals_cast[i * iteration_stride + id] = bf16::from_float(vals_arr[i]);
    }
    if ((high_index) < row_stride) {
        vals_arr[iterations] = vals_arr[iterations] * rsqrt(variance);
        vals_arr[iterations] = vals_arr[iterations] *
            bf16::to_float(gamma[high_index]) +
...
#pragma unroll
    ...
    // }
    for (int i = 0; i < iterations; i++) {
        vals_f[i] = residual_cast[i * iteration_stride + id].convert<float>(); // __half22float2(residual_cast[i * iteration_stride + id]);
        sum += vals_f[i].x();
        sum += vals_f[i].y();
    }
    // if ((high_index) < row_stride) {
    ...
    // }
    if ((high_index) < row_stride) {
        vals_f[iterations] = residual_cast[high_index].convert<float>(); // __half22float2(residual_cast[high_index]);
        sum += vals_f[iterations].x();
        sum += vals_f[iterations].y();
        iterations++;
    }
    ...
    half2 variance_h = vec<float, 2>({variance, variance}).convert<half>(); // __float2half2_rn(variance);
    ...
    // }
    if ((high_index) < row_stride) {
        half2 vals_arr = vals_f[iterations].convert<half>(); // __float22half2_rn(vals_f[iterations]);
        vals_arr = vals_arr * rsqrt(variance_h);
        vals_arr = vals_arr * gamma_cast[high_index] + beta_cast[high_index];
        vals_cast[high_index] = vals_arr;
    }
...
void launch_bias_residual_layer_norm<float>(float* vals,
                                            ...
                                            float* means);
template
void launch_bias_residual_layer_norm<bf16>(bf16* vals,
                                           const bf16* residual,
                                           const bf16* gamma,
                                           const bf16* beta,
                                           float epsilon,
...
void launch_bias_residual_layer_norm(T* vals,
    ...
    /*
      DPCT1049:10: The workgroup size passed to the SYCL kernel may exceed the limit. To get
      * the device limit, query info::device::max_work_group_size. Adjust the workgroup size if
      * needed.
      */
    stream->submit([&](handler& cgh) {
        accessor<float, 1, access_mode::read_write, access::target::local>
            shr_acc_ct1(range<1>(MAX_SG_NUM /*MAX_WARP_NUM*/), cgh);
                                           ...
                                           half* vars)
{
    ...
    range<3> block_dim(1, 1, threads);
    /*
      DPCT1049:11: The workgroup size passed to the SYCL kernel may exceed the limit. To get
      * the device limit, query info::device::max_work_group_size. Adjust the workgroup size if
      * needed.
      */
    stream->submit([&](handler& cgh) {
                        ...
                        float *gamma_buffer)
{
    ...
    // betta_buffer[item_ct1.get_local_id(2)][item_ct1.get_local_id(1)] = betta_tmp;
    // gamma_buffer[item_ct1.get_local_id(2)][item_ct1.get_local_id(1)] = gamma_tmp;
    ...
    // float s1 = betta_buffer[item_ct1.get_local_id(1)][item_ct1.get_local_id(2)];
    // float s2 = gamma_buffer[item_ct1.get_local_id(1)][item_ct1.get_local_id(2)];
                              ...
                              float *gamma_buffer)
{
    ...
    for (int r = item_ct1.get_local_id(1); r < rows; r += TILE_DIM) {
        ...
        val = (val - bf16::to_float(means_cast[r])) *
            rsqrt(bf16::to_float(vars_cast[r]));
        betta_tmp += grad;
        gamma_tmp += (val * grad);
...
#pragma unroll
    ...
    for (int i = 0; i < iterations; i++) {
        sum += vals_hat_arr[i] * vals_arr[i] *
            sqrt(var_reg);           // dval_hat = gamma * (x - u) * out_grad
    ...
    if (sg.get_local_id() < warp_num)
        sum = partialSum[sg.get_local_id()];
    ...
    for (int i = 0; i < iterations; i++)
        if constexpr(is_fuseadd) {
            inp_grad[i * iteration_stride + id] = (vals_arr[i] - sum)
                +out_grad_add[i * iteration_stride + id];
        } else {
            inp_grad[i * iteration_stride + id] = (vals_arr[i] - sum);
        }
    if ((high_index) < row_stride)
                        ...
                        float *partialSum)
{
    ...
    if constexpr(is_fuseadd) {
        out_grad_add_cast += (row * row_stride);
    }
        ...
        vals_hat_arr[iterations] =
            (invertible ?
             (bf16::to_float(vals_hat_cast[high_index]) -
              bf16::to_float(betta_cast[high_index])) / gamma_reg
             :
             bf16::to_float(vals_hat_cast[high_index]));
        iterations++;
    }
    ...
    for (int i = 0; i < iterations; i++)
        if constexpr(is_fuseadd) {
            inp_grad_cast[i * iteration_stride + id] =
                bf16::from_float(
                    (vals_arr[i] - sum) +bf16::to_float(
                        out_grad_add_cast[i * iteration_stride + id]));
        } else {
            inp_grad_cast[i * iteration_stride + id] =
                bf16::from_float((vals_arr[i] - sum));
        }
    if ((high_index) < row_stride)
        if constexpr(is_fuseadd) {
            inp_grad_cast[high_index] =
                bf16::from_float(
                    (vals_arr[iterations] - sum) + bf16::to_float(
                        out_grad_add_cast[high_index]));
        } else {
            inp_grad_cast[high_index] =
                bf16::from_float((vals_arr[iterations] - sum));
        }
}
...
#pragma unroll
    ...
    /*
      DPCT1065:20: Consider replacing nd_item::barrier() with
      * sycl::nd_item::barrier(sycl::access::fence_space::local_space) for better performance, if
      * there is no access to global memory.
      */
    item_ct1.barrier();
    ...
    for (int i = 0; i < iterations; i++) {
        ...
        if constexpr(is_fuseadd) {
            inp_grad_h[i * iteration_stride + id] = temp +
                out_grad_add_h[i * iteration_stride + id];
        } else {
            inp_grad_h[i * iteration_stride + id] = temp;
        }
    }
                               ...
                               const T* betta)
{
    ...
    stream[0]->submit([&](handler& cgh) {
        accessor<float, 2, access_mode::read_write, access::target::local>
            betta_buffer(range<2>(MAX_SG_NUM /*MAX_WARP_NUM*/, MAX_SG_NUM1), cgh);
        accessor<float, 2, access_mode::read_write, access::target::local>
            gamma_buffer(range<2>(MAX_SG_NUM /*MAX_WARP_NUM*/, MAX_SG_NUM1), cgh);
    ...
    /*
      DPCT1049:24: The workgroup size passed to the SYCL kernel may exceed the limit. To get
      * the device limit, query info::device::max_work_group_size. Adjust the workgroup size if
      * needed.
      */
    stream[1]->submit([&](handler& cgh) {
        accessor<float, 1, access_mode::read_write, access::target::local>
            partialSum_acc_ct1(range<1>(MAX_SG_NUM /*MAX_WARP_NUM*/), cgh);
...
template
void launch_layerNorm_backward<bf16>(const bf16* out_grad,
                                     const bf16* vals_hat,
                                     const bf16* vars,
                                     const bf16* gamma,
                                     bf16* gamma_grad,
                                     ...
                                     const half* betta)
{
    ...
    /*
      DPCT1049:25: The workgroup size passed to the SYCL kernel may exceed the limit. To get
      * the device limit, query info::device::max_work_group_size. Adjust the workgroup size if
      * needed.
      */
    stream[1]->submit([&](handler& cgh) {
...
#pragma unroll
    ...
    /*
      DPCT1065:30: Consider replacing nd_item::barrier() with
      * sycl::nd_item::barrier(sycl::access::fence_space::local_space) for better performance, if
      * there is no access to global memory.
      */
    item_ct1.barrier();
    ...
    /*
      DPCT1065:32: Consider replacing nd_item::barrier() with
      * nd_item::barrier(sycl::access::fence_space::local_space) for better performance, if
      * there is no access to global memory.
      */
    item_ct1.barrier();
                               ...
                               queue* stream[2])
{
    ...
    range<3> block_dim2(1, 1, threads);
    /*
      DPCT1049:34: The workgroup size passed to the SYCL kernel may exceed the limit. To get
      * the device limit, query info::device::max_work_group_size. Adjust the workgroup size if
      * needed.
      */
    stream[1]->submit([&](handler& cgh) {
                                     ...
                                     queue* stream[2])
{
    ...
    range<3> block_dim2(1, 1, threads / 2);
    /*
      DPCT1049:35: The workgroup size passed to the SYCL kernel may exceed the limit. To get
      * the device limit, query info::device::max_work_group_size. Adjust the workgroup size if
      * needed.
      */
    stream[1]->submit([&](handler& cgh) {
...
/*
 * deprecated Yuankun Shi 2021/10/19
 */
// template <typename T>
// void LayerNormBackward1_fused_add(const T* out_grad1,
//                                   const T* out_grad2,
//                                   const T* vals_hat,
//                                   const T* gamma,
//                                   const T* betta,
...
//     /*
//       DPCT1065:36: Consider replacing nd_item::barrier() with
...
//     /*
//       DPCT1065:38: Consider replacing nd_item::barrier() with
//       * sycl::nd_item::barrier(sycl::access::fence_space::local_space) for better performance, if
//       * there is no access to global memory.
//       */
//     item_ct1.barrier();
...
/*
 * merged with LayerNormBackward2
 */
// void LayerNormBackward2_fused_add(const float* out_grad1,
...
//     float sum = 0;
//     for (int i = 0; i < iterations; i++) {
//         sum += vals_hat_arr[i] * vals_arr[i] * sqrt(var_reg);
//         vals_arr[i] *= rsqrt(var_reg);
//     }
 ...
 * merged with LayerNormBackward2
 */
// void LayerNormBackward2_fused_add(const half* out_grad1,
...
//     /*
//       DPCT1065:44: Consider replacing nd_item::barrier() with
//       * nd_item::barrier(sycl::access::fence_space::local_space) for better performance, if
//       * there is no access to global memory.
//       */
//     item_ct1.barrier();
...
//     /*
//       DPCT1065:46: Consider replacing nd_item::barrier() with
//       * sycl::nd_item::barrier(sycl::access::fence_space::local_space) for better performance, if
//       * there is no access to global memory.
//       */
//     item_ct1.barrier();
                                         ...
                                         const T* betta)
{
    ...
    range<3> block_dim2(1, 1, threads);
    /*
      DPCT1049:48: The workgroup size passed to the SYCL kernel may exceed the limit. To get
      * the device limit, query info::device::max_work_group_size. Adjust the workgroup size if
      * needed.
      */
    stream[1]->submit([&](handler& cgh) {
...
template
void launch_layerNorm_backward_fused_add<bf16>(const bf16* out_grad1,
                                               ...
                                               const half* betta)
{
    ...
    range<3> block_dim2(1, 1, threads / 2);
    /*
      DPCT1049:49: The workgroup size passed to the SYCL kernel may exceed the limit. To get
      * the device limit, query info::device::max_work_group_size. Adjust the workgroup size if
      * needed.
      */
    stream[1]->submit([&](handler& cgh) {
...
/*
 * merged with LayerNormBackward2 Yuankun SHi 2021/10/19
 */
// void LayerNormBackward2_fused_add(const float* out_grad1,
//                                   const float* out_grad2,
//                                   const float* X_vals,
...
/*
 * merged with LayerNormBackward2 Yuankun SHi 2021/10/20
 */
// void LayerNormBackward2_fused_add(const half* out_grad1,
//                                   const half* out_grad2,
//                                   const half* X_vals,
...
//     /*
//       DPCT1065:56: Consider replacing nd_item::barrier() with
//       * nd_item::barrier(access::fence_space::local_space) for better performance, if
//       * there is no access to global memory.
//       */
//     item_ct1.barrier();
                                         ...
                                         queue* stream[2])
{
    ...
    range<3> block_dim2(1, 1, threads);
    /*
      DPCT1049:58: The workgroup size passed to the SYCL kernel may exceed the limit. To get
      * the device limit, query info::device::max_work_group_size. Adjust the workgroup size if
      * needed.
      */
    stream[1]->submit([&](handler& cgh) {
                                               ...
                                               queue* stream[2])
{
    ...
    range<3> block_dim2(1, 1, threads / 2);
    /*
      DPCT1049:59: The workgroup size passed to the SYCL kernel may exceed the limit. To get
      * the device limit, query info::device::max_work_group_size. Adjust the workgroup size if
      * needed.
      */
    stream[1]->submit([&](handler& cgh) {
