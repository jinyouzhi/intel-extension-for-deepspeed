...
class SYCLOpBuilder(OpBuilder):
    ...
    def jit_load(self, verbose=True):
        ...
        try:
            ...
            from intel_extension_for_pytorch.xpu.utils import load
...
def sycl_kernel_path(code_path):
    ...
    intel_link_path = os.path.join(os.path.dirname(FusedAdamBuilder().deepspeed_src_path("")),INTEL_DIR_NAME)
    if not os.path.exists(intel_link_path):
    # Create intel directory and link:  deepspeed/ops/INTEL_DIR_NAME-->../../INTEL_DIR_NAME
        intel_dir_path = os.path.join(os.path.dirname(intel_link_path),"../../"+INTEL_DIR_NAME)
  
        os.mkdir(intel_dir_path)
        os.symlink("../../"+INTEL_DIR_NAME,intel_link_path,True)
        print("Create home directory and link for intel:{}-->{}".format(intel_link_path , intel_dir_path))
    
    import filecmp
    if(os.path.exists(abs_target_path) and filecmp.cmp(abs_target_path,abs_source_path) ):
        print("skip copy, {} and {} have the same content".format(abs_source_path,abs_target_path))
        return rel_target_path
    
    print ("Copying SYCL kernel file from {} to {}".format(abs_source_path, abs_target_path))
