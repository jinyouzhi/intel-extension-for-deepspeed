...
inline int onednn_matmul(sycl::queue *handle,
    ...
    if constexpr(bmm) {
        src_md = dnnl::memory::desc(src_dims,
                                    dnnl::memory::data_type::bf16,
                                    trans_src ?
                                    dnnl::memory::format_tag::acb :
                                    dnnl::memory::format_tag::abc);
        wgt_md = dnnl::memory::desc(wgt_dims,
                                    dnnl::memory::data_type::bf16,
                                    trans_wgt ?
                                    dnnl::memory::format_tag::acb :
                                    dnnl::memory::format_tag::abc);
        dst_md = dnnl::memory::desc(dst_dims,
                                    dnnl::memory::data_type::bf16,
                                    dnnl::memory::format_tag::abc);
    } else {
        src_md = dnnl::memory::desc(src_dims,
...
int onednn_matmul_ex(sycl::queue *handle,
                     ...
                     const bf16* wgt_ptr,
                     bf16* dst_ptr) {
    onednn_matmul<false>(handle, trans_src, trans_wgt, m, n, k,
                         alpha, beta, src_ptr, wgt_ptr, dst_ptr, 1);
}
...
int onednn_batchgemm(sycl::queue *handle,
                     ...
                     bool trans_wgt,
                     int batch) {
    onednn_matmul<true>(handle, trans_src, trans_wgt, m, n, k,
                        alpha, beta, src_ptr, wgt_ptr, dst_ptr, batch);
}
