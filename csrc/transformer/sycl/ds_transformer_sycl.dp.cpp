...
BertTransformerLayer<T>::BertTransformerLayer(int layer_id,
    ...
    : _layer_id(layer_id),
      ...
      //_onemklQ(::SyclContext::Instance().GetOneMKLQ()),
...
{
    // no need for it on dpcpp
    // TODO guokai remove completly if validation passes
    //if (std::is_same<T, sycl::half>::value) cublasSetMathMode(_onemklQ, CUBLAS_TENSOR_OP_MATH);
}
...
void BertTransformerLayer<T>::Forward(int bsz,
                                      ...
                                      T* ff2_inp_ptr)
{
    //TODO guokai revive this line cublasSetStream(_onemklQ, _stream);
    ...
    if (_pre_or_postLayerNorm) {
        if (_layer_norm.UseMean()) {
            ...
            // input_ptr[bsz_seq], norm_w_ptr[_seq_length], norm_b_ptr[_seq_length] --> inp_norm_ptr[bsz_seq]
            _layer_norm.Forward(
                bsz_seq, inp_norm_ptr, input_ptr, norm_w_ptr, norm_b_ptr, _stream, true);
        }
    }
...
void BertTransformerLayer<T>::Backward(int bsz,
                                       ...
                                       T* grad_norm_b_ptr)
{
    // TODO guokai revive this cublasSetStream(_onemklQ, _stream);
    if (!_stochastic_mode) _stream->wait();
    ...
    if (_attn_prob_dropout.HasDropout()) {
        ...
        _attn_context.Backward(
            bsz_heads, buf_2, v_tf_ptr, soft_out_ptr, _onemklQ, buf_3, ff2_buf);
    }
    ...
    // the size of input (buf_1) relates to the last argument (trans_count), in this case, buf_1 connected
    // with buf_2 and buf_3 are all inputs
    ...
    // TODO: gma need this wait to keep nan appears in norm_w and norm_b, need further investigation and remove this wait
    _stream->wait();
}
                             ...
                             bool stochastic_mode)
{
    ...
    std::string dtype = (std::is_same<T, sycl::half>::value) ? "half" :
