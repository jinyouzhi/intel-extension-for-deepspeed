...
template <typename T>
void Transpose_Kernel(const T* inp, T* out, int row_width, int col_width, sycl::nd_item<3> item_ct1, T *data_block)
{
                       ...
                       sycl::queue* stream)
{
    ...
    stream->submit([&](sycl::handler&cgh) {
        ...
        cgh.parallel_for(
            sycl::nd_range<3>(grid_dim, block_dim), [=](sycl::nd_item<3> item_ct1) {
                Transpose_Kernel<sycl::half>(inp_mat, out_mat, cols, rows, item_ct1, data_block_acc_ct1.get_pointer());
            });
    });
}
    ...
    stream->submit([&](sycl::handler&cgh) {
        ...
        cgh.parallel_for(
            sycl::nd_range<3>(grid_dim * block_dim, block_dim), [=](sycl::nd_item<3> item_ct1) {
                Transpose_Kernel<float>(inp_mat, out_mat, cols, rows, item_ct1, data_block_acc_ct1.get_pointer());
            });
    });
}
                                  ...
                                  sycl::queue* stream)
{
    ...
    stream->submit([&](sycl::handler&cgh) {
        cgh.parallel_for(
            sycl::nd_range<3>(grid_dim * block_dim, block_dim), [=](sycl::nd_item<3> item_ct1) {
                transform_0213<float>(output, vals, hidden_dim, seq_length, heads, head_ext, item_ct1);
            });
    });
}
                                  ...
                                  sycl::queue* stream)
{
    ...
    stream->submit([&](sycl::handler&cgh) {
        cgh.parallel_for(
            sycl::nd_range<3>(grid_dim * block_dim, block_dim), [=](sycl::nd_item<3> item_ct1) {
                transform_0213<bf16>(output, vals, hidden_dim, seq_length, heads, head_ext, item_ct1);
            });
    });
}
                                   ...
                                   sycl::queue* stream)
{
    ...
    stream->submit([&](sycl::handler&cgh) {
        cgh.parallel_for(
            sycl::nd_range<3>(grid_dim * block_dim, block_dim), [=](sycl::nd_item<3> item_ct1) {
                transform_0213<sycl::half>(output, vals, hidden_dim, seq_length, heads, head_ext, item_ct1);
            });
    });
}
...
#pragma unroll
    for (int iter = 0; iter < 2; iter++) {
        ...
        int iter_offset =
            (iter_row % item_ct1.get_local_range(1)) * d2_out_stride + (iter_row / item_ct1.get_local_range(1)) * matrix_stride;
        output_vec[out_index + iter_offset] =
            in_data[iter_row * d2_stride + d3 + (d2 % 2) * (d1_stride * item_ct1.get_local_range(0))];
    }
}
                                           ...
                                           int trans_count)
{
    ...
    stream->submit([&](sycl::handler&cgh) {
        cgh.parallel_for(
            sycl::nd_range<3>(grid_dim * block_dim, block_dim), [=](sycl::nd_item<3> item_ct1) {
                bias_add_transform_0213<float>(output, vals, bias, hidden_dim, seq_length, heads, head_ext, item_ct1);
            });
    });
                                           ...
                                           int trans_count)
{
    ...
    stream->submit([&](sycl::handler&cgh) {
        cgh.parallel_for(
            sycl::nd_range<3>(grid_dim * block_dim, block_dim), [=](sycl::nd_item<3> item_ct1) {
                bias_add_transform_0213<bf16>(output, vals, bias, hidden_dim, seq_length, heads, head_ext, item_ct1);
            });
    });
    //bias_add_transform_0213<float><<<grid_dim, block_dim, 0, stream>>>(
    //    output, vals, bias, hidden_dim, seq_length, heads, head_ext);
                                            ...
                                            int trans_count)
{
    ...
    if (hidden_dim > 128 || hidden_dim < 16) {
        ...
        stream->submit([&](sycl::handler&cgh) {
            cgh.parallel_for(
                sycl::nd_range<3>(grid_dim * block_dim, block_dim), [=](sycl::nd_item<3> item_ct1) {
                    bias_add_transform_0213<sycl::half>(output, vals, bias, hidden_dim, seq_length, heads, head_ext, item_ct1);
                });
        });
        ...
        stream->submit([&](sycl::handler&cgh) {
            sycl::accessor<sycl::float4, 1, sycl::access::mode::read_write, sycl::access::target::local>
                data_block_acc_ct1(sycl::range<1>(3072), cgh);
            cgh.parallel_for(
                sycl::nd_range<3>(grid_dim * block_dim, block_dim), [=](sycl::nd_item<3> item_ct1) {
                    bias_add_transform_0213_v2(output, vals, bias, hidden_dim, seq_length, heads, item_ct1, data_block_acc_ct1.get_pointer());
                });
        });
                             ...
                             sycl::nd_item<3> item_ct1)
{
    ...
    int d2 = (item_ct1.get_local_id(1) + item_ct1.get_local_range(1) * item_ct1.get_group(1)) % seq_length;
                                         ...
                                         sycl::nd_item<3> item_ct1)
{
    ...
    int d0 = item_ct1.get_group(2);                                                  // Batch
    int d1 = item_ct1.get_local_id(1) + (item_ct1.get_group(0) % head_ext) * (heads / head_ext);  // Head
...
#pragma unroll
    for (int iter = 0; iter < 2; iter++) {
        ...
        in_data[d3 + iter_offset + (iter_row / item_ct1.get_local_range(1) + (d1 % 2) * item_ct1.get_local_range(0)) * d1_stride] =
                                     ...
                                     int trans_count)
{
    ...
    if (hidden_dim > 128 || hidden_dim < 16) {
        ...
        stream->submit([&](sycl::handler&cgh) {
            cgh.parallel_for(
                sycl::nd_range<3>(grid_dims * block_dims, block_dims), [=](sycl::nd_item<3> item_ct1) {
                    transform4d_0213<sycl::half>(out, in, heads, seq_length, hidden_dim, head_ext, item_ct1);
                });
        });
    } else {
        ...
        stream->submit([&](sycl::handler&cgh) {
            ...
            cgh.parallel_for(
                sycl::nd_range<3>(grid_dims * block_dims, block_dims), [=](sycl::nd_item<3> item_ct1) {
                transform4d_0213_v2(out, in, heads, seq_length, hidden_dim, item_ct1, data_block_acc_ct1.get_pointer());
                });
        });
