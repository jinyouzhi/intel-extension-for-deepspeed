...
normalize_forward(const int batch,
                  const int seq_len,
    ...
    Normalize_Layer<T> _norm(
        typename Normalize_Layer<T>::Config(batch,
                                            seq_len,
                                            hidden_size,
                                            epsilon,
                                            true,
    ...
    if (wmean)
        _norm.ForwardCheckpoint(bsz_seq,
                                output_ptr,
                                residual_ptr,
                                gamma_ptr,
                                betta_ptr,
...
template<typename T>
std::vector<torch::Tensor>
normalize_backward(const int batch,
                   const int seq_len,
                   const int hidden_size,
