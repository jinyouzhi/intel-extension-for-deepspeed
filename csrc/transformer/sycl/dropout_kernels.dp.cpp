                    ...
                    nd_item<3> item_ct1)
{
    ...
    /*
    DPCT1050:15: The template argument of the RNG engine could not be deduced. You need to
     * update this code.
    */
    oneapi::mkl::rng::device::philox4x32x10<4> engine(seed.first,
                                                      {idx*4, seed.second});
    oneapi::mkl::rng::device::uniform<> distr;
...
#pragma unroll
    ...
    DPCPP_1D_KERNEL_LOOP(j, N / unroll_factor)
    {
        ...
        uint8_t m[unroll_factor];
        /*
        DPCT1007:2: Migration of this CUDA API is not supported by the Intel(R) DPC++
         * Compatibility Tool.
        */
        float4 rand = oneapi::mkl::rng::device::generate(distr, engine);
        m[0] = (uint8_t)(rand.x() > ratio);
    ...
    if (N > high_index) {
        /*
        DPCT1007:3: Migration of this CUDA API is not supported by the Intel(R) DPC++
         * Compatibility Tool.
        */
        float4 rand = oneapi::mkl::rng::device::generate(distr, engine);
        float* rand_data = &(rand.x());
                        ...
                        nd_item<3> item_ct1)
{
    ...
    DPCPP_1D_KERNEL_LOOP(j, N / unroll_factor)
    {
        ...
        #else // TODO this path has a bug that would produce NaN, needs investigation
        out_cast[i] = bf16::from_float(
            mask[i] ? bf16::to_float(Xdata_cast[i]) * scale : 0.0);
        out_cast[i + 1] = bf16::from_float(
            mask[i + 1] ? bf16::to_float(Xdata_cast[i + 1]) * scale : 0.0);
        out_cast[i + 2] = bf16::from_float(
            mask[i + 2] ? bf16::to_float(Xdata_cast[i + 2]) * scale : 0.0);
        out_cast[i + 3] = bf16::from_float(
            mask[i + 3] ? bf16::to_float(Xdata_cast[i + 3]) * scale : 0.0);
        #endif
    }
    int high_index = ((((N / unroll_factor) - 1) / item_ct1.get_local_range().get(2) + 1) *
                      (unroll_factor * item_ct1.get_local_range().get(2))) +
    ...
    if (N > high_index) {
        for (int i = high_index; i < N; i++) {
            ...
            #else // TODO this path has a bug that would produce NaN, needs investigation
            out_cast[i] = bf16::from_float(
                mask[i] ? bf16::to_float(Xdata[i]) * scale : 0.0);
            #endif
        }
    }
}
...
#pragma unroll
    ...
    const half h_scale =
        vec<float, 1>{scale}.convert<half, rounding_mode::automatic>()[0];
    const half h_zero =
        vec<float, 1>{0.0}.convert<half, rounding_mode::automatic>()[0];
    ...
    if (bwd)
        stream->submit([&](handler & cgh) {
            cgh.parallel_for(nd_range<3>(grid_dim * block_dim, block_dim),
                             [=](nd_item<3> item_ct1) {
                                 dropout_kernel_bwd(total_count,
                                                    ratio,
                                                    vals,
                                                    out,
                                                    mask,
...
void dropout_grad_kernel(const int N, const float scale, float* Xdata,
                         uint8_t* mask, nd_item<3> item_ct1)
{
    DPCPP_1D_KERNEL_LOOP(i, N)
    {
        Xdata[i] *= scale * mask[i];
    }
}

void dropout_grad_kernel(const int N, const float scale, bf16* Xdata,
                         uint8_t* mask, nd_item<3> item_ct1)
{
    ushort *Xdata_cast = reinterpret_cast<ushort *>(Xdata);
    DPCPP_1D_KERNEL_LOOP(i, N)
    {
        Xdata_cast[i] = bf16::from_float(
            bf16::to_float(Xdata_cast[i]) * scale * mask[i]);
    }
}

void dropout_grad_kernel(const int N, const float scale, half* Xdata,
                         uint8_t* mask, nd_item<3> item_ct1)
{
    const half2 h_scale =
        float2{scale, scale}.convert<half, rounding_mode::rte>();
...
template <typename T>
void launch_dropout_grad(T* vals, uint8_t* mask, int total_count,
                         float ratio, queue* stream)
{
    /*
     * Dropout.Backward0
     */
    ...
    stream->submit([&](handler& cgh) {
        cgh.parallel_for(
            ...
            [=](nd_item<3> item_ct1) {
                dropout_grad_kernel(total_count,
                                    scale,
                                    vals,
                                    mask,
                                    item_ct1);
...
{
    const float scale = 1. / (1. - ratio);
    size_t idx = item_ct1.get_group(2) * item_ct1.get_local_range().get(2) + item_ct1.get_local_id(2);
    int tid = item_ct1.get_local_id(2) % (dim / unroll_factor);

    /*
    DPCT1050:16: The template argument of the RNG engine could not be deduced. You need to
     * update this code.
    */
    oneapi::mkl::rng::device::philox4x32x10<4> engine(seed.first,
                                                      {idx *4, seed.second});
    ...
    {
        /*
        DPCT1007:6: Migration of this CUDA API is not supported by the Intel(R) DPC++
         * Compatibility Tool.
        */
        float4 rand = oneapi::mkl::rng::device::generate(distr, engine);
        uint32_t m_32;
                    ...
                    nd_item<3> item_ct1)
{
    ...
    /*
    DPCT1050:17: The template argument of the RNG engine could not be deduced. You need to
     * update this code.
    */
    oneapi::mkl::rng::device::philox4x32x10<4> engine(seed.first,
                                                      {idx *4, seed.second});
    ...
    DPCPP_1D_KERNEL_LOOP(j, N)
    {
        ...
        */
        ...
        data_h_0.x() = vec<float, 1>{data_h_0.x() * scale * m[0]}
                           .convert<half, rounding_mode::automatic>()[0];
        data_h_0.y() = vec<float, 1>{data_h_0.y() * scale * m[1]}
                           .convert<half, rounding_mode::automatic>()[0];
        data_h_1.x() = vec<float, 1>{data_h_1.x() * scale * m[2]}
                           .convert<half, rounding_mode::automatic>()[0];
        data_h_1.y() = vec<float, 1>{data_h_1.y() * scale * m[3]}
                           .convert<half, rounding_mode::automatic>()[0];
    ...
    if (N > high_index) {
        ...
        */
        ...
        for (int i = high_index; i < N; i++) {
            ...
            uint8_t m = (uint8_t)(rand_data[k++] > ratio);
            Xdata[i] = vec<float, 1>{x_data * scale * m}
                           .convert<half, rounding_mode::automatic>()[0];
            mask[i] = m;
        }
    }
                    ...
                    nd_item<3> item_ct1)
{
    ...
    if (N > high_index) {
        ...
        int k = 0;
        for (int i = high_index; i < N; i++) {
            float x_data = bf16::to_float(input_cast[i]) +
                bf16::to_float(bias_cast[i % dim]);
            uint8_t m = (uint8_t)(rand_data[k++] > ratio);
            x_data = x_data * scale * m;
                    ...
                    nd_item<3> item_ct1)
{
    ...
    /*
    DPCT1050:19: The template argument of the RNG engine could not be deduced. You need to
     * update this code.
    */
    oneapi::mkl::rng::device::philox4x32x10<4> engine(seed.first,
                                                      {idx*4, seed.second});
    ...
    if (N > high_index) {
        /*
        DPCT1007:13: Migration of this CUDA API is not supported by the Intel(R) DPC++
         * Compatibility Tool.
        */
        float4 rand = oneapi::mkl::rng::device::generate(distr, engine);
        float* rand_data = &(rand.x());
