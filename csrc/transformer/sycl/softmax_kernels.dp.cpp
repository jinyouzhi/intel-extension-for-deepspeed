                  ...
                  float *partialSum)
{
    ...
    if (seq_length > tbSize) {
        ...
        /*
          DPCT1065:2: Consider replacing nd_item::barrier() with
          * nd_item::barrier(access::fence_space::local_space) for better performance if
          * there is no access to global memory.
          */
        item_ct1.barrier();
...
#ifndef __STOCHASTIC_MODE__
        /*
          DPCT1065:3: Consider replacing nd_item::barrier() with
          * nd_item::barrier(access::fence_space::local_space) for better performance if
          * there is no access to global memory.
          */
        item_ct1.barrier();
                         ...
                         queue* stream)
{
    ...
    range<3> block_dim(
        ...
        1,
        seq_length4 > threads
        ? ((sequence_length + subblock_max_workload - 1) / subblock_max_workload * threads)
        : threads);
    int iterations =
    ...
    if (sequence_length <= 8)
        ...
        if (sequence_length <= 512) {
            stream->submit([&](handler & cgh){
                ...
                cgh.parallel_for(nd_range<3>(grid_dim * block_dim, block_dim),
                                 [=](nd_item<3> item_ct1) [[intel::reqd_sub_group_size(MAX_SG_NUM)]] {
                                     attn_softmax<32, (threads / 128), 128>(
                                         vals,
                                         attn_mask,
                                         heads,
                                         seq_length4,
...
template
void launch_attn_softmax<bf16>(bf16* vals,
                               const bf16* attn_mask,
                               int batch_size,
                               int heads,
                               int sequence_length,
...
template
void launch_attn_softmax<half>(half* vals,
                               const half* attn_mask,
                               int batch_size,
                               int heads,
                               int sequence_length,
...
void softmax_backward_kernel(T* out_grad, const T* soft_inp, int seq_length,
                             nd_item<3> item_ct1, float *partialSum)
{
    int sg_num =
        item_ct1.get_local_range().get(2) >> 5;  // sg-count = num_threads / SG_SIZE (32)
...
#pragma unroll
    ...
    if (seq_length > tbSize) {
        ...
        /*
          DPCT1065:4: Consider replacing nd_item::barrier() with
          * nd_item::barrier(access::fence_space::local_space) for better performance if
          * there is no access to global memory.
          */
        item_ct1.barrier();
                                     ...
                                     queue* stream)
{
    ...
    if (seq_length <= 32)
        /*
          DPCT1049:5: The workgroup size passed to the SYCL kernel may exceed the limit. To
          * get the device limit, query info::device::max_work_group_size. Adjust the workgroup size
          * if needed.
          */
        stream->parallel_for(nd_range<3>(grid_dim * block_dim, block_dim),
                             [=](nd_item<3> item_ct1) [[intel::reqd_sub_group_size(MAX_SG_NUM)]] {
                                 softmax_backward_kernel_v2<T, 1>(out_grad, soft_inp, seq_length, item_ct1);
                             });
    else if (seq_length <= 64)
        /*
          DPCT1049:6: The workgroup size passed to the SYCL kernel may exceed the limit. To
          * get the device limit, query info::device::max_work_group_size. Adjust the workgroup size
          * if needed.
          */
        stream->parallel_for(nd_range<3>(grid_dim * block_dim, block_dim),
                             [=](nd_item<3> item_ct1) [[intel::reqd_sub_group_size(MAX_SG_NUM)]] {
                                 softmax_backward_kernel_v2<T, 2>(out_grad, soft_inp, seq_length, item_ct1);
                             });
    else if (seq_length <= 128)
        /*
          DPCT1049:7: The workgroup size passed to the SYCL kernel may exceed the limit. To
          * get the device limit, query info::device::max_work_group_size. Adjust the workgroup size
          * if needed.
          */
        stream->parallel_for(nd_range<3>(grid_dim * block_dim, block_dim),
                             [=](nd_item<3> item_ct1) [[intel::reqd_sub_group_size(MAX_SG_NUM)]] {
                                 softmax_backward_kernel_v2<T, 4>(out_grad, soft_inp, seq_length, item_ct1);
                             });
    else if (seq_length <= 256)
        /*
          DPCT1049:8: The workgroup size passed to the SYCL kernel may exceed the limit. To
          * get the device limit, query info::device::max_work_group_size. Adjust the workgroup size
          * if needed.
          */
        stream->parallel_for(nd_range<3>(grid_dim * block_dim, block_dim),
                             [=](nd_item<3> item_ct1) [[intel::reqd_sub_group_size(MAX_SG_NUM)]] {
                                 softmax_backward_kernel_v2<T, 8>(out_grad, soft_inp, seq_length, item_ct1);
                             });
    else if (seq_length <= 384)
        /*
          DPCT1049:9: The workgroup size passed to the SYCL kernel may exceed the limit. To
          * get the device limit, query info::device::max_work_group_size. Adjust the workgroup size
          * if needed.
          */
        stream->parallel_for(nd_range<3>(grid_dim * block_dim, block_dim),
                             [=](nd_item<3> item_ct1) [[intel::reqd_sub_group_size(MAX_SG_NUM)]] {
                                 softmax_backward_kernel_v2<T, 12>(out_grad, soft_inp, seq_length, item_ct1);
                             });
    else if (seq_length <= 512)
        /*
          DPCT1049:10: The workgroup size passed to the SYCL kernel may exceed the limit.
          * To get the device limit, query info::device::max_work_group_size. Adjust the workgroup
          * size if needed.
          */
        stream->parallel_for(nd_range<3>(grid_dim * block_dim, block_dim),
                             [=](nd_item<3> item_ct1) [[intel::reqd_sub_group_size(MAX_SG_NUM)]] {
                                 softmax_backward_kernel_v2<T, 16>(out_grad, soft_inp, seq_length, item_ct1);
                             });
    else if (seq_length <= 768)
        /*
          DPCT1049:11: The workgroup size passed to the SYCL kernel may exceed the limit.
          * To get the device limit, query info::device::max_work_group_size. Adjust the workgroup
          * size if needed.
          */
        stream->parallel_for(nd_range<3>(grid_dim * block_dim, block_dim),
                             [=](nd_item<3> item_ct1) [[intel::reqd_sub_group_size(MAX_SG_NUM)]] {
                                 softmax_backward_kernel_v2<T, 24>(out_grad, soft_inp, seq_length, item_ct1);
                             });
    else if (seq_length <= 1024)
        /*
          DPCT1049:12: The workgroup size passed to the SYCL kernel may exceed the limit.
          * To get the device limit, query info::device::max_work_group_size. Adjust the workgroup
          * size if needed.
          */
        stream->parallel_for(nd_range<3>(grid_dim * block_dim, block_dim),
                             [=](nd_item<3> item_ct1) [[intel::reqd_sub_group_size(MAX_SG_NUM)]] {
                                 softmax_backward_kernel_v2<T, 32>(out_grad, soft_inp, seq_length, item_ct1);
                             });
        ...
        stream->parallel_for(nd_range<3>(grid_dim * block_dim, block_dim),
                             [=](nd_item<3> item_ct1) [[intel::reqd_sub_group_size(MAX_SG_NUM)]] {
                                 softmax_backward_kernel_v2<T, 64>(out_grad, soft_inp, seq_length, item_ct1);
                             });
    else
        throw std::runtime_error(
